...
export default async function(ctx) {
  ...
  // 🚀 获取到 IP 信息后立刻异步翻译城市名（与测速并行）
  const ipwhoPromise = (ctx.response?.json) 
    ? ctx.response.json().then(async (ipInfo) => {
        if (ipInfo.city_name) {
          ipInfo.city_name_cn = await translateCity(ipInfo.city_name, ctx);
        }
        return ipInfo;
      }).catch(() => ({})) 
    : Promise.resolve({});
  ...
  return {
    body: {
      ...
      // 🌟 优先显示中文城市名，若翻译失败则无缝降级回原英文
      ...(ipInfo.city_name ? { '城市': ipInfo.city_name_cn || ipInfo.city_name } : {}),
