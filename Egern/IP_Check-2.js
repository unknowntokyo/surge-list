...
export default async function(ctx) {
  ...
  let obj;
  try {
    // 💡 修正：Egern 环境下必须使用 JSON.parse 解析字符串 body
    obj = JSON.parse(ctx.response.body);
  } catch (e) {
    return; 
  }
  
  // 2. 实时测速配置
  const MB = 1; // 💡 使用 1MB 文件确保在几百毫秒内秒开，防止因下载文件过大导致面板转圈
  ...
  try {
    ...
    // 发起实时测速请求
    // 💡 修正：去掉了 policy 参数，使其自动匹配当前代理节点的路由规则
    await ctx.http.get(SPEED_TEST_URL, {
      headers: { 'Cache-Control': 'no-cache' },
      timeout: 3000 // 3秒超时保护，防止卡死
    });
  ...
  // 💡 修正：通过直接重写 ctx.response.body 将修改后的 JSON 字符串返回给 Egern
