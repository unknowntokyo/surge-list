...
export default async function(ctx) {
  ...
  try {
    ...
    try {
      // 直接请求 5MB 文件，设定 1.5 秒硬超时
      await ctx.http.get("https://speed.cloudflare.com/__down?bytes=5242880", {
        headers: { 'Cache-Control': 'no-cache' },
        timeout: 5000 
      });
      // 精简计算公式：5MB * 8 = 40 Mbit
      speedMbps = `${(40 / ((Date.now() - startTime) / 1000)).toFixed(1)} Mbps`;
    } catch(e) {
      // 捕获超时或请求失败，保持"测速失败 ⚠️"
    }
    ...
    return {
      body: {
        ...
        ...(obj.city_name && { "城市": obj.city_name }), // 精简的按需写入写法
