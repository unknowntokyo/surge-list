...
async function translateCity(text, ctx) {
  if (!text) return '';
  try {
    // 使用谷歌翻译公共 Web API，强制指定源语言自动，目标语言为简体中文 (zh-CN)
    const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=zh-CN&dt=t&q=${encodeURIComponent(text)}`;
    const res = await ctx.http.get(url, { timeout: 2000 }); // 设置 2 秒超时保护
    const resText = await res.text();
    const json = JSON.parse(resText);
    
    // 谷歌接口固定结构：第一层嵌套的第一个数组元素即为翻译结果
    return json?.[0]?.[0]?.[0] || text;
  } catch (e) {
    // 容错机制：如果网络抖动或节点断连导致翻译失败，直接返回原英文，确保面板不卡死
    return text;
  }
}
...
export default async function(ctx) {
  ...
  // 🚀 核心优化：获取到 IP 信息后立刻触发异步翻译，与下方的测速并行，不占额外时间
  const ipwhoPromise = (ctx.response?.json) 
    ? ctx.response.json().then(async (ipInfo) => {
        if (ipInfo.city_name) {
          // 将翻译后的中文挂载到新属性 city_name_cn 上
  ...
  return {
    body: {
      'IP地址': ipInfo.ip || '未知',
      '地区': codeMap[ipInfo.country_code] || ipInfo.country_code || '未知',
      // 🌟 兜底逻辑：优先显示中文城市名，如果因网络问题翻译失败，则无缝切换回原英文名
