const WEIGHTS = { "春节": 10, "国庆": 9, "元旦": 7, "清明": 7, "端午": 7, "中秋": 7 };
const TITLES = [
  "距离放假，还要摸鱼多少天？", "坚持住，就快放假啦！", "上班好累呀，下顿吃啥？", "努力，我还能加班24小时！", "躺平中，等放假", "施主请回，此饼不吃", "只有摸鱼才是赚老板的钱", "小乌龟慢慢爬", "加油，明天会更好！", "生活本该如此轻松", "好累，但还能坚持一会儿", "快放假啦，期待放松的时光", "给自己加个鸡腿！", "佛系上班，一切随缘", "我的理想是：不上班还有钱", "放弃幻想，认清现状，低调搬砖", "生活碎片，拼凑成诗", "慢慢走，沿途的花都开了", "没什么期待，也就没什么失望", "所谓的成长，就是学会不抱希望", "今天的任务是：不干活！", "用力生活，用力摸鱼"
];

const Lunar = {
  info: [0x04bd8,0x04ae0,0x0a570,0x054d5,0x0d260,0x0d950,0x16554,0x056a0,0x09ad0,0x055d2,0x04ae0,0x0a5b6,0x0a4d0,0x0d250,0x1d255,0x0b540,0x0d6a0,0x0ada2,0x095b0,0x14977,0x04970,0x0a4b0,0x0b4b5,0x06a50,0x06d40,0x1ab54,0x02b60,0x09570,0x052f2,0x04970,0x06566,0x0d4a0,0x0ea50,0x06e95,0x05ad0,0x02b60,0x186e3,0x092e0,0x1c8d7,0x0c950,0x0d4a0,0x1d8a6,0x0b550,0x056a0,0x1a5b4,0x025d0,0x092d0,0x0d2b2,0x0a950,0x0b557,0x06ca0,0x0b550,0x15355,0x04da0,0x0a5b0,0x14573,0x052b0,0x0a9a8,0x0e950,0x06aa0,0x0aea6,0x0ab50,0x04b60,0x0aae4,0x0a570,0x05260,0x0f263,0x0d950,0x05b57,0x056a0,0x096d0,0x04dd5,0x04ad0,0x0a4d0,0x0d4d4,0x0d250,0x0d558,0x0b540,0x0b6a0,0x195a6,0x095b0,0x049b0,0x0a974,0x0a4b0,0x0b27a,0x06a50,0x06d40,0x0af46,0x0ab60,0x09570,0x04af5,0x04970,0x064b0,0x074a3,0x0ea50,0x06b58,0x05ac0,0x0ab60,0x096d5,0x092e0,0x0c960,0x0d954,0x0d4a0,0x0da50,0x07552,0x056a0,0x0abb7,0x025d0,0x092d0,0x0cab5,0x0a950,0x0b4a0,0x0baa4,0x0ad50,0x055d9,0x04ba0,0x0a5b0,0x15176,0x052b0,0x0a930,0x07954,0x06aa0,0x0ad50,0x05b52,0x04b60,0x0a6e6,0x0a4e0,0x0d260,0x0ea65,0x0d530,0x05aa0,0x076a3,0x096d0,0x04afb,0x04ad0,0x0a4d0,0x1d0b6,0x0d250,0x0d520,0x0dd45,0x0b5a0,0x056d0,0x055b2,0x049b0,0x0a577,0x0a4b0,0x0aa50,0x1b255,0x06d20,0x0ada0,0x14b63,0x09370,0x049f8,0x04970,0x064b0,0x168a6,0x0ea50,0x06b20,0x1a6c4,0x0aae0,0x092e0,0x0d2e3,0x0c960,0x0d557,0x0d4a0,0x0da50,0x05d55,0x056a0,0x0a6d0,0x055d4,0x052d0,0x0a9b8,0x0a950,0x0b4a0,0x0b6a6,0x0ad50,0x055a0,0x0aba4,0x0a5b0,0x052b0,0x0b273,0x06930,0x07337,0x06aa0,0x0ad50,0x14b55,0x04b60,0x0a570,0x054e4,0x0d160,0x0e968,0x0d520,0x0daa0,0x16aa6,0x056d0,0x04ae0,0x0a9d4,0x0a2d0,0x0d150,0x0f252,0x0d520],
  term(y, n) {
    return new Date((31556925974.7 * (y - 1900)) + [0,21208,42467,63836,85337,107014,128867,150921,173149,195551,218072,240693,263343,285989,308563,331033,353350,375494,397447,419210,440795,462224,483532,504758][n-1] * 60000 + Date.UTC(1900,0,6,2,5));
  },
  lDays(y) {
    let s = 348;
    for (let i = 0x8000; i > 0x8; i >>= 1) s += (this.info[y - 1900] & i) ? 1 : 0;
    return s + ((this.info[y - 1900] & 0xf) ? ((this.info[y - 1900] & 0x10000) ? 30 : 29) : 0);
  }
};

const _lunarOffsets = new Int32Array(2101);
let _acc = 0;
for (let i = 1900; i <= 2100; i++) {
  _lunarOffsets[i] = _acc;
  _acc += Lunar.lDays(i);
}

const bjDate = new Date();
const Y = bjDate.getFullYear();
const todayMs = Date.UTC(Y, bjDate.getMonth(), bjDate.getDate());

const l2sMs = (y, m, d) => {
  let off = _lunarOffsets[y] || 0;
  const info = Lunar.info[y - 1900];
  const lp = info & 0xf;
  for (let i = 1; i < m; i++) {
    off += (info & (0x10000 >> i)) ? 30 : 29;
  }
  if (lp > 0 && m > lp) {
    off += (info & 0xf) ? ((info & 0x10000) ? 30 : 29) : 0;
  }
  return Date.UTC(1900, 0, 31) + (off + d - 1) * 86400000;
};

const legalMap = new Map();
const todayFests = [];

const FIXED_FESTS = [
  { name: "元旦",   type: "solar", m: 1,  d: 1,  dur: 1 }, 
  { name: "春节",   type: "lunar", m: 1,  d: 1,  dur: 3 },
  { name: "清明",   type: "term",  n: 7,         dur: 1 },
  { name: "五一", type: "solar", m: 5,  d: 1,  dur: 1 },
  { name: "老婆🎂",   type: "solar", m: 5,  d: 12, dur: 1 },
  { name: "端午",   type: "lunar", m: 5,  d: 5,  dur: 1 },
  { name: "中秋",   type: "lunar", m: 8,  d: 15, dur: 1 },
  { name: "国庆",   type: "solar", m: 10, d: 1,  dur: 3 },
  { name: "Ann",    type: "solar", m: 10, d: 4,  dur: 1 },
  { name: "D.O.B",  type: "lunar", m: 12, d: 3,  dur: 1 }
];

for (let i = 0; i < 2; i++) {
  const currentYear = Y + i;
  
  for (let j = 0, len = FIXED_FESTS.length; j < len; j++) {
    const f = FIXED_FESTS[j];

    if (todayFests.indexOf(f.name) !== -1) continue;

    let festMs = 0;
    if (f.type === "solar") {
      festMs = Date.UTC(currentYear, f.m - 1, f.d);
    } else if (f.type === "lunar") {
      festMs = l2sMs(currentYear, f.m, f.d);
    } else { 
      const t = Lunar.term(currentYear, f.n);
      festMs = Date.UTC(t.getFullYear(), t.getMonth(), t.getDate());
    }

    const diff = Math.round((festMs - todayMs) / 86400000);

    if (diff <= 0) {
      if (diff >= 1 - f.dur) {
        todayFests.push(f.name);
        legalMap.delete(f.name); 
      }
    } else if (!legalMap.has(f.name)) {
      legalMap.set(f.name, {
        name: f.name,
        diff: diff,
        priority: WEIGHTS[f.name] || 1
      });
    }
  }
}

const sortedList = Array.from(legalMap.values())
  .sort((a, b) => (a.diff - b.diff) || (b.priority - a.priority));

const rawText = sortedList.slice(0, 4)
  .map(i => `${i.name} ${i.diff}天`)
  .join("，");

const todayNoticeText = todayFests.length > 0 
  ? `🎉 ${todayFests.slice(0, 2).join("·")}${todayFests.length > 2 ? "…" : ""}` 
  : "";

const isWeekend = (bjDate.getDay() === 0 || bjDate.getDay() === 6);
const iconColor = todayFests.length > 0 ? "#FF3B30" : (isWeekend ? "#34C759" : "#8E8E93");

let panelContentLines = [];
if (todayNoticeText) {
  panelContentLines.push(`${todayNoticeText}\n`);
}
panelContentLines.push(rawText ? `${rawText}` : "近期暂无倒计时");

$done({
  title: TITLES[Math.floor(Math.random() * TITLES.length)],
  content: panelContentLines.join(""),
  icon: "hourglass",
  "icon-color": iconColor
});